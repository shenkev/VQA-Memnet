...
def generate_synthetic_data(num_species, num_attributes, num_clues_per_species):
    ...
    for attribute_id in range(num_attributes):
        ...
        selected_species = set(
            random.sample(list(range(num_species)), int(proportion_of_species_with_attribute * num_species)))
        ...
        for species_id in selected_species:
            species_clues[species_id].append([attribute_id+1])


            # proportion_of_clues_with_attribute = random.uniform(proportion_of_clues_with_attribute_low,
            #                                                     proportion_of_clues_with_attribute_high)
            # selected_clues = set(random.sample(list(range(num_clues_per_species)), max(1, int(
            #     round(proportion_of_clues_with_attribute * num_clues_per_species)))))
            #
...
def generate_simple_questions(base_questions, num_attributes):
    ...
    for q in base_questions:
        ...
        simple_questions.append(
            (species_id, binary_vector(num_attributes, [attribute_id]), binary_vector(2, [answer_id])))
...
def generate_and_or_questions(base_questions, num_questions_to_generate, num_species, num_attributes):
    ...
    for _ in xrange(num_questions_to_generate / 2):
        ...
        num_false_attributes_for_question = random.randint(1,
                                                           min(len(false_attributes), question_num_attribute_max / 2))
        false_target_attributes = random.sample(list(false_attributes), num_false_attributes_for_question)

        and_questions_false.append((species_id,
                                    binary_vector(num_attributes, true_target_attributes + false_target_attributes),
                                    binary_vector(2, [0])))
    ...
    for _ in xrange(num_questions_to_generate / 2):
        ...
        num_false_attributes_for_question = random.randint(0,
                                                           min(len(false_attributes), question_num_attribute_max / 2))
        false_target_attributes = random.sample(list(false_attributes), num_false_attributes_for_question)

        or_questions_true.append((species_id,
                                  binary_vector(num_attributes, true_target_attributes + false_target_attributes),
                                  binary_vector(2, [1])))
...
if __name__ == '__main__':
    ...
    pickle.dump(
        [species_clues, simple_questions_train, args.num_species, args.num_attributes, args.num_clues_per_species],
        open("synthetic_single_data_{}_species_{}_attributes_train.pckl".format(args.num_species,
                                                                                        args.num_attributes,
                                                                                        args.num_clues_per_species),
             "wb"))
    pickle.dump(
        [species_clues, simple_questions_test, args.num_species, args.num_attributes, args.num_clues_per_species],
        open("synthetic_single_data_{}_species_{}_attributes_test.pckl".format(args.num_species,
                                                                                       args.num_attributes,
                                                                                       args.num_clues_per_species),
             "wb"))
