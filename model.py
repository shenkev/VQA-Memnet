...
def compute_evidence_weights(e, q):
    i_temperature = 10  # you gota play with this value
...
def prune_data(evidence):
    min_index = evidence.size(1)

    for i in range(evidence.size(0)):
        for j in range(evidence.size(1)-1, -1, -1):
            if torch.sum(evidence[i][j]).data[0] < 1:
                min_index = min(j, min_index)
            else:
                break

    return evidence.narrow(1, 0, min_index).contiguous()
